    ...
    if (req.method === 'GET' && req.url !== '/dapr/config') { // note: the first GET request sent to the application is from the sidecar at path /dapr/config; we do not currently have a proper implementation for that path
        ...
        if (key != null) {
            try {
                ...
                // inform topic TOPIC_NAME on pubsub PUBSUB_NAME of the name that was requested - using a proper JSON-formatted text
                await daprclient.pubsub.publish(PUBSUB_NAME, TOPIC_NAME, JSON.stringify({"data": key})); // publish the name to the pubsub topic
